...
public class ReceiptMaker {
	...
	public final String SENTINEL = "checkout";
	...
	public double getMaxPrice(){if(numItemsPurchased == 0) {
		return (0);
	}
	...
	public double getMeanPrice(){if(numItemsPurchased == 0) {
		return (0);
	}
			return getSubtotal()/numItemsPurchased;
	}

	public double getTaxOnSubtotal(){if(numItemsPurchased == 0) {
		return (0);
	}
			return getSubtotal() * TAX_RATE;
	}

	public double getTotal(){if(numItemsPurchased == 0) {
		return (0);
	}
	...
	public void displayAllItemsWithPrices(){
		System.out.println("\n---------------RECEIPT BREAKDOWN---------------");
		for(int i=0; i<numItemsPurchased; i++){
			System.out.printf("Item #%02d   Name: %12s | Price: $ %.2f\n",(i+1), itemNames[i], itemPrices[i]);
		}
	}
